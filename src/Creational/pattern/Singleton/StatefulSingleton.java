 ...
 * 有状态的可变的单例对象往往当做状态库使用，比如一个单例对象持有一个int型的属性，用来给一个系统提供一个数值惟一的序列号码。
 ...
 * <p>
 * 因为单例类是局限于某一个JVM中的，所以在任何使用了EJB、RMI和JINI技术的分布式系统中(多JVM)，应当避免使用有状态的单例类。
 * 同样对于同一个JVM中的多个类加载器，当它们加载同一个类时，会出现多个实例，
